...
- `43f06f3` T2：助手 markdown 渲染 + 输入框自增高（Cancel 按钮暂缓，见 T2.1）
- README 增补 WebUI 章节（构建/运行/发布）+ workspace 表加 kestrel-server / console

**夜间总结**：T0/T1/T2 全绿（fmt+clippy+test+deny + console build），端到端已验证，
全部提交并推送。创新点文档待你早上评审拍板（头号候选 The Glass Engine）。下一步需要你
的方向：从 innovation-brainstorm.md 的岔路里选，然后才好开工新特性（那些要先立 ADR）。
