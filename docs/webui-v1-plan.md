...
- [ ] T1.1  console：`ApprovalRequired` 弹权限模态 -> Approve / Deny
- [ ] T1.2  server：`GET /api/sessions`（列表）+ `GET /api/sessions/:id/events`（回放）
- [ ] T1.3  console：会话列表 + 点击载入历史（回放视图）
- [ ] T1.4  console：SSE 断线重连
- [ ] T1.5  提交并推送 T1

## T2 — 打磨

- [ ] T2.1  console：Cancel（停止）按钮 -> `Op::Cancel`
- [ ] T2.2  console：助手文本 markdown 渲染
- [ ] T2.3  console：自动滚动、空状态、错误 toast
- [ ] T2.4  console：键盘（Enter 发送 / Shift+Enter 换行）
- [ ] T2.5  提交并推送 T2
...
- [ ] 深挖创新点，写成 `docs/innovation-brainstorm.md`，供早上评审。

## 进度日志

（每次提交追加一行：commit 短哈希 + 完成项）
