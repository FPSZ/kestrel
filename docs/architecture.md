...
graph TB
    subgraph frontends["前端层（薄壳，只渲染事件）"]
        ...
        WEB["kestrel-server + console<br/>(axum SSE + React WebUI)"]
    end
    ...
    CLI -->|Op| LOOP
    WEB -->|"Op (POST /api/ops)"| LOOP
    LOOP -->|"Event (mpsc, 有序)"| CLI
    LOOP -->|"Event (SSE /api/events)"| WEB
    LOOP --> CTX
    LOOP --> PERM
...
│   ├── kestrel-cli/              # 前端：极简 REPL 起步，M2 引入 ratatui TUI
│   └── kestrel-server/           # 前端适配器：axum，SSE 推事件 + POST 收 Op（ADR-0007）
├── console/                      # WebUI（React+Vite+Tailwind），纯前端，不进 Rust 依赖图
├── profiles/                     # 内置模型 profile（qwen3-8b.toml 等，探针可覆盖）
└── tests/replays/                # 录制的回放测试 fixture（.jsonl）
```

7 个 Rust crate + 1 个前端应用（对比：codex ~100、goose 13）。每个 crate 的 `lib.rs` 顶部用模块级文档注释声明其**职责边界与禁止依赖**，CI 用 `cargo-deny` 强制依赖白名单。`console/` 通过 HTTP 契约与 `kestrel-server` 通信，不在 Rust 依赖图内。
...
| **WebUI 个人版**（进行中） | `kestrel-server`（axum SSE 事件 + POST Op）+ `console`（React 扁平深色壳）：聊天回合流式 + 权限模态 + 会话回放；单人本机、无认证（ADR-0007） | 独奏，可视化 |
...
| **朋友版（原 v2）** | 在 `kestrel-server` 上叠：认证 + 多会话隔离 + TLS——给朋友用 | 不改 core 一行 |
...
- [ADR-0010 模型启动器：把模型作为 agent 的一部分来启动/监督](adr/0010-model-launcher.md)
