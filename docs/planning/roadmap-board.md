...
| **地基（G）· 横切约定** | i18n + 设计令牌 + 事件日志前向兼容 + core 确定性 + 密钥 + 错误分类         | 与机组无关     | `[x]` 已交付                         |
...
- [X] G4  protocol/core：错误分类 `ErrorCode`（backend/tool/store/cancelled/internal）+ `EventPayload::Error.code`，前端按 code 本地化（`error.*` catalog），message 作开发向细节
- [X] G5  console：i18n 脚手架（`t(key,params)` + `src/i18n/{en-US,zh-CN}.json`）+ 迁移硬编码文本（chat/sidebar/sessions/topbar/settings；launcher 自带 EN/ZH catalog）
- [X] G6  kestrel-cli：用户可见 REPL/slash/事件标签走 `i18n::t` catalog（en-US/zh-CN，`KESTREL_LOCALE`/`LANG` 判定），tracing 保持英文
- [X] G7  console：设计令牌审计——全量颜色走 `@theme` 令牌，消灭最后一处 `text-white`（改 `text-desktop`）
- [X] G8  CI：i18n 硬编码门禁（`console/scripts/check-i18n.mjs` 扫硬编码 CJK）+ console job（lint:i18n → build）
- [X] G9  ADR-0011 + 实现：`EVENT_LOG_SCHEMA_VERSION` + `.schema_version` 标记 + 容忍解析（`#[serde(other)] Unknown` 变体、坏行 skip+warn、未知字段忽略）
- [X] G10 密钥：`SecretString` 型（脱敏 Debug/Serialize、无 Display、`expose()` 唯一出口）贯穿 config/loadout/backend/组装根
- [X] G11 locale 格式化（`formatTime/formatDateTime/formatNumber` Intl，键 `getLocale()`）+ 事件 UTC 时间戳（`Event.ts` 由 store 边缘盖章，core 不读时钟）
- [X] G12 ADR-0009：存储位置定为 OS 标准目录（`.kestrel/` opt-in + 版本化迁移）
- [X] G13 kestrel-store：改用 OS 标准目录（`directories` crate）+ `layout_version` 迁移钩子 + 从旧 `./sessions` 迁移（ADR-0009）
- [X] G14 fmt/clippy(-D warnings)/test(workspace)/deny + console(lint:i18n+build) 全绿；提交并推送地基段
...
- `2026-07-03` 地基（G）收尾：G4 结构化 ErrorCode + G5 console i18n 迁移 + G6 CLI catalog +
  G7 设计令牌审计 + G8 CI 硬编码门禁 + G9 事件日志前向兼容（ADR-0011）+ G10 SecretString +
  G11 locale 格式化 + UTC 时间戳边缘盖章。G13 OS 标准目录早已落地。fmt/clippy(-D)/test(workspace)/
  deny + console(lint:i18n+build) 全绿。同轮先修：主机感知 system prompt（避免模型吐 Linux 指令）+
  图片粘贴多模态 + 会话列表按最近活跃排序
